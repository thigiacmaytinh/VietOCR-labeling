...
namespace TGMTcs
{
    public class ArduinoEventArgs : EventArgs
    {
        ...
        public ArduinoEventArgs(string msg, string _error = "")
        {
            message = msg;
            error = _error;
        }
    ...
    public class TGMTarduino
    {
        ...
        private void port_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (message == "")
            {
                onMessageReceived?.Invoke(this, new ArduinoEventArgs("", "string empty"));
                return;
            }
            ...
            try
            {
                foreach (ManagementObject item in searcher.Get())
                {
                    ...
                    if (config != null)
                    {
                        ...
                        result.baudRate = int.Parse(config["BaudRate"].ToString());
                    }
                    else
                    {
                        if (item["MaxBaudRate"] != null)
                            result.baudRate = int.Parse(item["MaxBaudRate"].ToString());
                    }
                }
            }
